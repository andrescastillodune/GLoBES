...
char MYFILE[]="/home/dell/Documents/SN/globes-3.2.17/examples/Esteril/NSI.dat";
char AEDLFILE[]="/home/dell/Documents/SN/globes-3.2.17/examples/DUNE_16/DUNE_GLoBES.glb";
FILE *outfile = NULL;
...
{ 
  double thetheta13, thetheta14, x, y, res;
  
  /* Define standard oscillation parameters (cf. hep-ph/0405172v5) */
  double true_theta12 = asin(sqrt(0.3))*M_PI/180.0;
  double true_theta13 = 10.0*M_PI/180.0;
  double true_theta23 = M_PI/4;
  double true_deltacp = 270.0*M_PI/180.0;
  double true_sdm = 7.9e-5;
  double true_ldm = 2.3e-3;
  ...
  /* Sterile parameters*/
  double true_TH_14=0.0*M_PI/180.0;
  
  /* Initialize libglobes */
  glbInit(argv[0]);
 
  /* Register non-standard probability engine. This has to be done
   * before any calls to glbAllocParams() or x`glbAllocProjections() */
  int _n_flavors=4;
  int _rotation_order[][2]={ {3,4}, {2,3}, {2,4}, {1,4} , {1,3}, {1,2}};
  int _phase_order[]={ -1, 0, -1, 0, 0, -1};
   
 
  
  
  snu_init_probability_engine(_n_flavors,_rotation_order,_phase_order);
  ...
  glbInitExperiment("/home/dell/Documents/SN/globes-3.2.17/examples/DUNE_16/DUNE_GLoBES.glb",&glb_experiment_list[0],&glb_num_of_exps); 
  ...
  glbSetOscParams(true_values,true_TH_14, GLB_TH_14);   /* Non-standard parameter */
  glbSetDensityParams(true_values,1.0,GLB_ALL);
  glbDefineParams(test_values,true_theta12,true_theta13,true_theta23,
                              true_deltacp,true_sdm,true_ldm);
  glbSetOscParams(test_values,true_sigma_E, dmsq[2]);   /* Non-standard parameter */
  glbSetOscParams(test_values,true_TH_14, th[1][4]);  
  ...
  glbSetOscParams(input_errors, 0.0, dmsq[2]);      /* Non-standard parameter */
  glbSetOscParams(input_errors, 0.0, th[1][4]); 
  ...
  glbDefineProjection(th13_sigma_projection, GLB_FREE, GLB_FREE, GLB_FREE,
    GLB_FREE, GLB_FREE, GLB_FREE);
  glbSetDensityProjectionFlag(th13_sigma_projection, GLB_FREE, GLB_ALL);
  glbSetProjectionFlag(th13_sigma_projection, GLB_FREE, dmsq[2]);
  glbSetProjectionFlag(th13_sigma_projection, GLB_FREE, th[1][4]); /* Non-standard parameter */
 ...
 /* Intitialize output */
  ...
  double th13_lower  = 2.4e-4;
  double th13_upper  = 30;
  double th13_steps  = 20;
  double delta_lower = 4*M_PI/180.;
  double delta_upper = 200*M_PI/180.;
  ...
  for(this_th13=th13_lower; this_th13<=th13_upper; this_th13+=(th13_upper-th13_lower)/th13_steps)
  {
    for(this_delta=delta_lower; this_delta<=delta_upper; this_delta+=(delta_upper-delta_lower)/delta_steps)
    {
      ...
      glbSetOscParams(test_values, this_th13, dmsq[2]);
      glbSetOscParams(test_values, this_delta, th[1][4]);

      /* Compute chi^2 assuming the normal mass hierarchy in the fit */
      res1 = glbChiSys(test_values, GLB_ALL, GLB_ALL);
      fprintf(outfile, "%g %g %g\n", sin(this_delta)*sin(this_delta), this_th13, res1);
    }
    fprintf(outfile, "\n");
  }
  fclose(outfile);
